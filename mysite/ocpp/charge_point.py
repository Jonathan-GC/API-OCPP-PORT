...
class Prueba (AsyncWebsocketConsumer):
    
    def __init__(self, miVar, **kwds):
        super().__init__()
        self.miVar = miVar
    ...
    async def connect(self):
        #self.chager_name = self.scope['url_route']['kwargs']['chager_name']
        #self.inicializar(self.chager_name)
        await self.accept()
        
        print(f"Cargador {self.chager_name} Connected")

    # Send message to WebSocket
    async def sendText(self):
        await self.send("matalabula" + self.nombre)
